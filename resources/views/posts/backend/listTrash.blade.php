...
@section('content')
    <div class="content-wrapper" style="min-height: calc(100vh - 200px);margin-bottom:-30px">
        <section class="content-header">
            <h1>
                Simple Posts
                <small>preview of simple posts</small>
            </h1>
            <ol class="breadcrumb">
        ...
        <div class="row nav-tabs">
            <div class="col-sm-6">
                <ul class="nav display-flex" id="myTab" role="tablist">
                    <li class="nav-item">
                    <a class="nav-link"  href="{{ route('dashboard.post.index') }}">Lists Of Post</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('dashboard.post.listTrash') }}">Trash</a>
                    </li>
                </ul>
            </div>
            <div class="col-sm-6 justify-content-end display-flex ml--15">
                <form action="{{ route('dashboard.post.seach') }}" style="margin-bottom:0px" method="GET">
    ...
    <form action="{{ route('dashboard.post.bulk') }}" method="POST">
        ...
        <div class="row bg-white font-size-13 mx-15 py-10 mt-15 mb-3">
            ...
            <div class="col-sm-6 justify-content-end display-flex">
                <a href="{{ route('dashboard.post.create') }}" class="bg-36c6d3 px-10 mr-10 color-white"><i class="fa fa-plus"></i> Create</a>
            ...
            @if ($trashs->count() > 0)
                @foreach ($trashs as $trash)
                    <tr class="child">
                        ...
                        <td class="text-center vertical-align-middle"><img src="{{ env('APP_URL') . Storage::url($trash->getThumbnail()) }}" alt="#" width="50px"></td>
                        <td class=" vertical-align-middle"><a href="#" title="{{ $trash->getTitle() }}">{{ $trash->getTitle() }}</a></td>
    ...
    @foreach ($trashs as $trash)
        <form action="{{ route('dashboard.post.restore',$trash->getId()) }}" method="POST" style="width:0px;height:0px">
            @csrf
            @method('POST')
            <input type="submit" value="Restore" style="visibility:hidden" id="restore-{{ $trash->getId() }}">
        </form>
        <form action="{{ route('dashboard.post.delete',$trash->getId()) }}" method="POST" style="width:0px;height:0px">
            @csrf
            @method('DELETE')
