...
@section('content')
    <div class="content-wrapper" style="min-height: calc(100vh - 200px);margin-bottom:-30px">
        <section class="content-header">
            <h1>
                Simple Pages
                <small>preview of simple pages</small>
            </h1>
            <ol class="breadcrumb">
        ...
        <div class="row nav-tabs">
            <div class="col-sm-6">
                <ul class="nav display-flex" id="myTab" role="tablist">
                    <li class="nav-item">
                    <a class="nav-link"  href="{{ route('dashboard.page.index') }}">Lists Of Post</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('dashboard.page.listTrash') }}">Trash</a>
                    </li>
                </ul>
            </div>
            ...
            @if ($trashs->count() > 0)
                @foreach ($trashs as $trash)
                    <tr class="child">
                        ...
                        <td class="text-center vertical-align-middle"><img src="{{ env('APP_URL') . Storage::url($trash->getImage()) }}" alt="#" width="50px"></td>
                        ...
                        <td class="text-center vertical-align-middle">{{ $trash->getTemplate() }}</td>
    ...
    @foreach ($trashs as $trash)
        <form action="{{ route('dashboard.page.restore',$trash->getId()) }}" method="POST" style="width:0px;height:0px">
            @csrf
            @method('POST')
            <input type="submit" value="Restore" style="visibility:hidden" id="restore-{{ $trash->getId() }}">
        </form>
        <form action="{{ route('dashboard.page.delete',$trash->getId()) }}" method="POST" style="width:0px;height:0px">
            @csrf
            @method('DELETE')
