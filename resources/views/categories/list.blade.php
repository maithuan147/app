...
@section('content')
                    ...
                    @foreach ($categories as $category)
                        <tr class="child">
                            ...
                            <td class="text-center vertical-align-middle"> 
                                <a href="{{ route('dashboard.category.edit',$category->getId()) }}" class="btn btn-primary">{{ __('Edit') }}</a>
    ...
    @foreach ($categories as $category)
        <form action="{{ route('dashboard.category.delete',$category->getId()) }}" method="POST" style="width:0px;height:0px">
            @csrf
            @method('DELETE')
