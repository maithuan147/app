...
@section('content')
    <div class="content-wrapper" style="min-height: calc(100vh - 200px);margin-bottom:-30px">
        <section class="content-header">
            ...
            <div class="row nav-tabs">
                ...
                <div class="col-sm-6 justify-content-end display-flex ml--15">
                    <form action="{{ route('dashboard.product-tag.index') }}" style="margin-bottom:0px" method="GET">
        ...
        <form action="{{ route('dashboard.product-tag.bulk') }}" method="POST">
            @csrf
            @method('DELETE')
            <div class="row bg-white font-size-13 mx-15 py-10 mt-15 mb-3">
                ...
                <div class="col-sm-6 justify-content-end display-flex">
                    <a href="{{ route('dashboard.product-tag.create') }}" class="bg-36c6d3 px-10 mr-10 color-white"><i class="fa fa-plus"></i> {{ __('Create') }}</a>
                    ...
                    @foreach ($tags as $tag)
                        <tr class="child">
                            ...
                            <td class="text-center vertical-align-middle"> 
                                <a href="{{ route('dashboard.product-tag.edit',$tag->getId()) }}" class="btn btn-primary">{{ __('Edit') }}</a>
    ...
    @foreach ($tags as $tag)
        <form action="{{ route('dashboard.product-tag.delete',$tag->getId()) }}" method="POST" style="width:0px;height:0px">
            @csrf
            @method('DELETE')
